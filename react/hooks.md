...
-  可以通过useEffect做到基本上和componentDidMount，componentWillUnmount 和 componentDidUpdate生命周期等价的
  ...
  # Hooks典型的使用场景
  ## 如何创建自定义Hooks？
  Hooks和普通函数在语义上的区别就是在函数中有没有用到其它Hooks。自定义的Hooks一定会使用其他Hooks，否则只是一个普通函数
...
#### 业务场景  
一般比较常用的业务场景有：网路异步请求、监听页面事件、复杂组件拆分等等
##### 复杂组件拆分
其中拆分复杂组件目的不一定是为了重用，也会出于便于维护和业务隔离的目的  
具体做法：尽量将相关的逻辑做成独立的 Hooks，然后在函数组中使用这些 Hooks，通过参数传递和返回值让 Hooks 之间完成交互

注意：我们要尽量避免将一个函数组件写的特别长，改变这个状况的关键在于开发思路的转变。要把Hooks看成普通的函数，能隔离的尽量去做隔离，使代码模块化。